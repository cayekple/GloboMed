...
class UpdateEmployeeActivity: AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		bundle?.let {
			...
			val employee = DataManger.fetchEmployee(databaseHelper, empId!!)
	...
	private fun saveEmployee() {
		...
		if (isValid) {
			...
			DataManger.updateEmployee(databaseHelper, updatedEmployee)
			...
			R.id.action_delete -> {
				...
				builder.setMessage(R.string.confirm_sure)
					.setPositiveButton(R.string.yes){dialog, eId ->
						val result = DataManger.deleteEmployee(databaseHelper, empId.toString() )
